...
async fn test_discover_metadata() {
    ...
    assert!(discovered.grant_types_supported.as_ref().unwrap().contains(&"authorization_code".to_string()));
    assert!(discovered.scopes_supported.as_ref().unwrap().contains(&"openid".to_string()));
    assert!(discovered.id_token_signing_alg_values_supported.as_ref().unwrap().contains(&"RS256".to_string()));
    assert!(discovered.code_challenge_methods_supported.as_ref().unwrap().contains(&"S256".to_string()));
}
...
async fn test_discover_error_cases() {
    ...
    // Test 404 response
    let _not_found_mock = server.mock("GET", "/.well-known/openid-configuration")
        .with_status(404)
        .create_async()
        .await;
    ...
    let _invalid_json_mock = server.mock("GET", "/.well-known/openid-configuration")
        .with_status(200)
        .with_header("content-type", "application/json")
    ...
    let _incomplete_mock = server.mock("GET", "/.well-known/openid-configuration")
        .with_status(200)
        .with_header("content-type", "application/json")
